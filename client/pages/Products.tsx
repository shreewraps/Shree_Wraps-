...
export default function Products() {
  ...
  let list = term ? searchProducts(term) : listByCategory(category);

  // Apply sorting based on selected option
  if (sortOption !== "none") {
    list = [...list]; // Create a copy to avoid mutating original
    switch (sortOption) {
      case "price-asc":
  ...
  useEffect(() => {
    const isInvalidPage =
      Number.isNaN(pageParam) || pageParam < 1 || pageParam > totalPages;
    console.debug("Products clamp effect", {
      pageParam,
      currentPage,
      isInvalidPage,
    ...
    if (isInvalidPage) {
      ...
      console.debug(
        "Products clamp navigating (replace) to",
        ...
        {list.length > perPage && (
          <div className="mt-6 flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Showing {startIdx + 1}–{Math.min(startIdx + perPage, list.length)}{" "}
              of {list.length}
            </div>
